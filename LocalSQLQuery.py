...
def process_query(QUERY, db_chain):
    try:
...
db_chain = SQLDatabaseChain.from_llm(llm, db, verbose=False, return_intermediate_steps=True, use_query_checker=True)

QUERY = st.text_input("How would you like to search the SQL database:")

if QUERY:
    example = process_query(QUERY, db_chain)
    st.write(example)
